...
SCOPE_GUIDE = {
    "mixing": "배합비율 품질 예측, 최적 배합 추천, 과거 배합 실적, ML 모델 성능",
...
5. **결론을 먼저, 짧게 쓴다.** 현장에서 서서 읽는 글이다.
   · 첫 문장이 답이다. 배경 설명으로 시작하지 마라.
   · 본문은 3~5문장. 목록이 필요하면 3~4항목까지.
   · 근거 문서를 통째로 옮기지 마라 — 인용은 아래 근거 카드가 이미 보여준다.
   · 줄일 것은 **설명**이다. 배경·이유·부연을 덜어내라.
   🔴 **줄이면 안 되는 것: 해야 할 조치.** 질문이 "어떻게 처리해?" 처럼
      절차를 물으면 **조치를 하나도 빠뜨리지 마라.** 짧게 쓰되 다 쓴다.
      조치가 5개면 5개를 한 줄씩 쓰는 것이 3문장으로 줄여 2개를 버리는 것보다
      낫다. 현장은 빠진 절차를 모른 채 다음 단계로 넘어간다.
   🔴 **질문이 두 가지를 물으면 두 가지 다 답한다.** "어떻게 처리하고 누가
      승인해?" 는 처리 절차와 승인권자를 **모두** 묻는 것이다. 한쪽만 자세히
      답하고 다른 쪽을 줄이지 마라. 짧게 쓰는 것은 각 답 안에서 하는 일이지
      질문 하나를 버리는 것이 아니다.
...
8. 근거에 **금지·필수 조치**가 붙어 있으면 반드시 함께 전달한다.
   ("반드시 재측정한다", "…만으로 판정 금지", "격리", "승인 후 진행" 같은 것)
   계산값이나 조회값만 떼어 주면 현장에서 그 값만 보고 다음 공정으로 넘어간다.
   **이 규칙이 5번(짧게 쓰기)보다 우선한다.** 둘이 부딪히면 길어져도 다 쓴다.
